...
class Resnet(nn.Module):
    def __init__(self, args):
        ...
        self.backbone_ct.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3,
                                         bias=False)

        self.backbone_pet = resnet50(pretrained=True)
        self.backbone_pet.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3,
                                         bias=False)
                            
        self.backbone_fusion = resnet50(pretrained=True)
        self.backbone_fusion.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3,
                                         bias=False)
...
if __name__ == '__main__':
    args = Args()
    ct_data = torch.rand(4, 1, 64, 64).cuda()
    pet_data = torch.rand(4, 1, 64, 64).cuda()
    fusion_data = torch.rand(4, 1, 64, 64).cuda()
